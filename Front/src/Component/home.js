...
import {Checkbox, Icon, Layout, Menu, Card, Modal, Form, Input, Row, Col} from 'antd';
...
export default class Home extends React.Component{
        ...
        if (Cookies.get('id') !== undefined && Cookies.get('id') !== "") {
            axios.get('http://localhost:5000/users/' + Cookies.get('id'),{ headers: {"Access-Control-Allow-Origin": "*"}})
                .then(res => {
                    ...
                    axios.get('http://localhost:5000/tasks/users/' + Cookies.get('id'),{ headers: {"Access-Control-Allow-Origin": "*"}})
                        .then(res => {
                            console.log(res.data);
    ...
    onChange(e) {
        ...
        switch (e.target.id) {
            case "Done":
                window.location = './home?show=Done';
                break;
            case "UnDone":
                window.location = "./home?show=UnDone";
                break;
            case "All":
    ...
    handleOkNewTaskModal = e => {
        if (document.getElementById('NewTitleTask').value !== "") {
                const task = {
                    ...
                    'user_id': this.currentUser._id.$oid
                };
                console.log(task);
